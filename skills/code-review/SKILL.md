...
**機械順位（任意・補助）**: 軸ごとの指摘を1行1件にして`review-rank.sh <file>`（要`TYPESAFE_API_KEY`）に渡すと、重大度順（0: 表現のみ〜3: ブロッカー、score ≥ 2.5 = ブロッカー）が出る。**結果は人が確認する**——順位はconsultであって判定の代わりにならず、軸を跨いだ順位付けには使わない。
