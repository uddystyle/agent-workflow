...
- Jev は routing 分類（`TaskClassifier`）では Choice のみ。提案の optional Nouls（`security_sensitive` / `migration` / `ambiguous_requirements`）は routing では未実装で、security/migration は keyword gate で拾う。Noul は router の外の Handoff 検証（`HandoffVerifier`・research.md §19）で実装・使用済みだが、これは consult boundary であり routing の判定には使わない。
