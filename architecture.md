...
確認日: 2026-09-18 / 2026-09-19 / 2026-09-20。事実根拠は [research.md](research.md)（§10 に実セッション実測、§13 に quota 実装、§14 に opt-out、§16 に project-local override 層、§17 に BudgetManager/GenerationFallback の interface 化、§19 に Handoff 検証）。本書で設計した router の prototype（`codex-jev-router.ts` / `codex-jev-router.json`）は実装・展開済みで、実測を含む。commit `3a873c1`（Jev model pin）、`c5ca162`（judgment 永続化）、`4f559ed`（実測記録）。将来境界のうち Budget state（`manual`/`estimated` と quota 永続化）、project-local config（opt-out / enabled override / route 上書き）、`BudgetManager` / `GenerationFallback` の interface 化は実装済み。router の外に、Jev を Noul の consult boundary（`HandoffVerifier`・handoff 検証）としても利用（observation-only・routing 不変）。
...
- Jev は routing 分類（`TaskClassifier`）では Choice のみ。提案の optional Nouls（`security_sensitive` / `migration` / `ambiguous_requirements`）は routing では未実装で、security/migration は keyword gate で拾う。Noul は router の外の consult boundary で実装・使用済み（Handoff 検証 `HandoffVerifier`・research.md §19、Herdr 状態解釈 `StateClassifier`・research.md §20）——どちらも routing の判定には使わない。
