...
- Jev は routing 分類（`TaskClassifier`）では Choice のみ。提案の optional Nouls（`security_sensitive` / `migration` / `ambiguous_requirements`）は routing では未実装で、security/migration は keyword gate で拾う。Noul / Score は router の外の consult boundary で実装・使用済み（Handoff 検証 `HandoffVerifier`・research.md §19、Herdr 状態解釈 `StateClassifier`・research.md §20、code-review findings 重大度順位 `SeverityRanker`・research.md §21）——どれも routing の判定には使わない。
