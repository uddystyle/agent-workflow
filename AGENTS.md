...
**Generated:** 2026-09-05T10:41:04Z
**Commit:** 5cced47
...
```sh
git log 5cced47..HEAD -- install.sh skills/ home/ tests/
```
...
- ⚠️ **同じ remote を指す clone がこのマシンに2つある**（移行中、D-21）。`main` が2つあり、
  別々に進みうる。マシンが配線されていない側で `install.sh` は止まる——正しい振る舞いである
