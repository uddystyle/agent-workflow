...
try:
    ...
    assert values[("theme.custom", "surface1")] == "#374145"
    assert values[("theme.custom", "surface_dim")] == "#15191b"
    assert values[("theme.custom", "overlay0")] == "#7a8478"
    assert values[("theme.custom", "overlay1")] == "#859289"
    assert values[("theme.custom", "subtext0")] == "#9da9a0"
    assert values[("theme.custom", "mauve")] == "#d699b6"
...
PY
then
	ok "管理対象の外部MCP serverはない"
else
	bad "外部MCP serverが設定されているか、MCPの安全側設定が足りない"
fi
