...
! grep -q '^pi install npm:pi-mcp-adapter$' "$CALL_LOG" || fail 'retired pi-mcp-adapter package was installed'
grep -q '^pi install npm:pi-web-access$' "$CALL_LOG" || fail 'managed pi-web-access package missing'
