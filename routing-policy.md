...
- **現在の stage（確認日 2026-09-19、レビュー実測は research.md §12・§15）:** `enabledRoutes: ["light", "hard"]`（stage 2）。light・hard 提案は適用され、very-hard 提案は NORMAL に落ちる。このとき decision entry に `rolloutGate: true` と `suggestedRouteId` が記録され、`/route report` の gated 集計で提案分布を観測できる。stage 1（light のみ）は誤ルーティングレビュー実測で前進を判断した（hard 提案 conf 0.97 の正判定・閾値 0.70 が低信頼提案を吸収）。very-hard は Jev の非決定性（§12 で 0.47–0.96）と gate keyword（security/migration/schema 等）の独立補完を考慮し、gated を維持する。§15 の追跡レビュー（2026-09-19）でも実 session の积み増し 0（gated 集計なし）で **stage 2 維持**を確認。
